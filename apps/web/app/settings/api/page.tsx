...
export default function ApiConnectPage() {
  ...
  const sdkExample = useMemo(() => `import { createKomponistClient } from '@komponist/sdk'
...
console.log(data.items[0].evidence)`, [])
  ...
  return <AppLayout>
    ...
    <div className="page-body max-w-3xl space-y-6">
      ...
      <div className="card"><div className="connect-head"><div><h2 className="text-h2 mb-2">TypeScript SDK</h2><p className="text-small text-muted">Use reviewed context from trusted server-side code with a typed <code>{'{ data, error }'}</code> contract.</p></div><button className="btn btn-secondary" onClick={() => copy(sdkExample)}>Copy example</button></div><pre>{sdkExample}</pre><p className="mt-3 text-caption text-muted">Never expose an organization API key in a browser or public mobile bundle.</p></div>
